 ...
 *
 * JD9165BA panel driver for JC1060P470C (1024×600 MIPI-DSI).
 * Init sequence sourced from official Guition/cheops reference firmware.
 */
...
{
    ESP_LOGI(TAG, "JD9165BA panel driver init");
...
/* ── Default init sequence for JD9165BA on JC1060P470C ── */
...
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ESP_RETURN_ON_ERROR(
            ...
            TAG, "send init cmd[%d] (0x%02x) failed", i, init_cmds[i].cmd);
        if (init_cmds[i].delay_ms) {
            vTaskDelay(pdMS_TO_TICKS(init_cmds[i].delay_ms));
        }
    }
    ESP_LOGI(TAG, "init commands sent OK");

    ESP_RETURN_ON_ERROR(jd9165->init(panel), TAG, "MIPI DPI panel init failed");
    return ESP_OK;
}
...
static esp_err_t panel_jd9165_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    ESP_RETURN_ON_FALSE(jd9165->io, ESP_ERR_INVALID_STATE, TAG, "invalid panel IO");
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(jd9165->io, cmd, NULL, 0),
...
static esp_err_t panel_jd9165_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    jd9165_panel_t *jd9165 = (jd9165_panel_t *)panel->user_data;
    ESP_RETURN_ON_FALSE(jd9165->io, ESP_ERR_INVALID_STATE, TAG, "invalid panel IO");
    uint8_t madctl = jd9165->madctl_val;
    if (mirror_x) madctl |=  JD9165_CMD_GS_BIT;
    else          madctl &= ~JD9165_CMD_GS_BIT;
    if (mirror_y) madctl |=  JD9165_CMD_SS_BIT;
    else          madctl &= ~JD9165_CMD_SS_BIT;
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(jd9165->io, LCD_CMD_MADCTL, (uint8_t[]){madctl}, 1),
        TAG, "mirror failed");
    jd9165->madctl_val = madctl;
    return ESP_OK;
}
